...
public class Pool<TElement, TId> where TElement : IPoolable<TElement, TId>
{
    ...
    public IReadOnlyDictionary<TId,IReadOnlyList<TElement>> FreeElements
    {
        get
        {
            var returner = new Dictionary<TId, IReadOnlyList<TElement>>();
            foreach (var element in _freeElements)
                returner.Add(element.Key, element.Value.ToArray());
            return returner;
        }
    }
    public IReadOnlyDictionary<TId,IReadOnlyList<TElement>> BusyElements
    {
        ...
        {
            var returner = new Dictionary<TId, IReadOnlyList<TElement>>();
            foreach (var element in _busyElements)
                returner.Add(element.Key, element.Value);
            return returner;
        }
    }
