...
public class UI_Controller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (UI_Activies.Count <= 0) Debug.LogWarning("No have active screen");
    ...
    {
        for (int i = 0; i < UI_Activies.Count; i++)
            UI_Activies[i].SetActive(false);

        UI_Activies = new List<GameObject>();
        
        UI_ScreenBase newScreen = UI_ScreenRepository.GetScreen(screen);
        newScreen.gameObject.SetActive(true);
        UI_Activies.Add(newScreen.gameObject);
    }    
